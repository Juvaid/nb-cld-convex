...
export default function Footer() {
    ...
    return (
        <footer className="bg-slate-900 text-white pt-20 sm:pt-24 pb-12 overflow-hidden relative">
            {/* Decorative background blur */}
            <div className="absolute top-0 right-0 w-[40%] h-[40%] bg-[#16a34a]/10 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2 opacity-30" />
                    ...
                    <div>
                        ...
                        <ul className="space-y-3 sm:space-y-4 text-xs sm:text-sm">
                            {['OEM Manufacturing', 'Private Labeling', 'Formulation R&D', 'Packaging Design', 'Contract Manufacturing'].map((link) => (
                                <li key={link}>
                                    <Link href="/services" className="text-white/60 hover:text-[#16a34a] transition-colors font-semibold flex items-center gap-2 group">
                                        {link}
                                        <ArrowUpRight className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-all -translate-x-1 group-hover:translate-x-0" />
                                    </Link>
                                </li>
                        ...
                        <ul className="space-y-4 sm:space-y-6">
                            {[
                                ...
                                <li key={i} className="flex gap-4 items-start group">
                                    <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-[#16a34a]/20 transition-colors">
