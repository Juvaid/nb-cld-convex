...
export default function ProductDetail({ slug, isModal = false, onClose }: ProductDetailProps) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Curating details...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="bg-slate-50 min-h-screen pb-20">
            {/* Top Navigation Bar */}
            <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-900/5 px-4 h-16 flex items-center justify-between">
                <div className="max-w-7xl mx-auto w-full flex items-center justify-between">
                    <Link href="/products" className="flex items-center gap-2 text-slate-500 hover:text-nb-green transition-all font-bold text-sm group">
                        <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
                        <span>Product Catalog</span>
                    </Link>
                    <button
                        onClick={handleInquiry}
                        className="bg-nb-green text-slate-900 px-6 py-2 rounded-xl font-black text-xs shadow-lg shadow-nb-green/20 active:scale-95 transition-all"
                    >
                        Bulk Inquiry
                    </button>
                </div>
