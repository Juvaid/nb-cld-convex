...
export default function BlogPostClient({ slug, initialSettings }: BlogPostClientProps) {
    ...
    if (markdownContent.startsWith("{") && markdownContent.includes('"content"')) {
        try {
            ...
            markdownContent = parsedData.content
                    ...
                    {blog.coverImage ? (
                        <>
                            <div className="absolute inset-0 z-0">
                                <img src={blog.coverImage} alt={blog.title} className="w-full h-full object-cover opacity-20 blur-xl scale-110" />
                                ...
                                {blog.author && (
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            <div className="text-[10px] font-black uppercase tracking-widest text-nb-green/80 leading-none mb-1">Written By</div>
                                            <div className="text-sm font-bold text-white">{blog.author}</div>
                                        </div>
                                    </div>
                                )}

                                {blog.author && <div className="hidden sm:block w-px h-8 bg-slate-700" />}

                                {(blog.publishedAt || blog._creationTime) && (
                                    <div className="text-left">
                                        <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 leading-none mb-1">Published On</div>
                                        <div className="text-sm font-bold text-white">
                                            {new Date(blog.publishedAt || blog._creationTime).toLocaleDateString(undefined, {
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric'
                                            })}
